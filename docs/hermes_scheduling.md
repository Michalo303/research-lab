...
Hermes derives the active research blocker from the structured rejection counts in the latest daily report before considering free-form risk prose. It maps only the supported walk-forward, drawdown, and cost-stress reasons. When canonical Knihomol inputs exist but yield no usable notes for that blocker, Hermes writes a `book_context_unavailable` terminal artifact and does not call the provider or change the queue.

Selected note IDs form an exact allowlist for that run. Every book-informed hypothesis must contain `used_note_ids` with one to five values from the allowlist. Missing, empty, malformed, invented, or non-allowlisted IDs are rejected and cannot change the queue. Before invoking the provider, Hermes also verifies that the exact selected book context and citation contract both survived final prompt construction; an ID without its evidence cannot authorize generation. Hermes never inserts or repairs citations after generation. The provider is invoked at most once; there is no automatic retry or fallback model. The daily EODHD stage runs only after Hermes atomically imports at least one new hypothesis.
