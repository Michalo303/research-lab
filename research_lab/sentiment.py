...
def run_apify_scaffold() -> dict:
    if not os.getenv("APIFY_TOKEN", "").strip():
        return {"coverage_status": "missing", "reason": "APIFY_TOKEN missing", "items": []}
    actor_id = os.getenv("APIFY_SENTIMENT_ACTOR_ID", "").strip()
    if not actor_id:
        return {"coverage_status": "missing", "reason": "APIFY_SENTIMENT_ACTOR_ID missing", "items": []}
    return {"coverage_status": "partial", "reason": "scaffold only", "items": []}
