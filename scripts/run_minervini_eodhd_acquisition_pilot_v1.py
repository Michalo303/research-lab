...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run one bounded Minervini EODHD acquisition pilot."
    )
    ...
    if isinstance(estimate, dict):
        print(
            "total_http_requests_lower_bound="
            f"{estimate['total_http_requests_lower_bound']}"
        )
        print(
            "total_http_requests_upper_bound="
            f"{estimate['total_http_requests_upper_bound']}"
        )
        print(
    ...
    return (
        0
        if result["status"] == "READY_FOR_WIDE_ACQUISITION_APPROVAL"
        else 2
    )
