...
def _request(assets=None, **overrides):
    ...
    bindings=[]
    for a in assets:
        a["normalized_bars_sha256"] = _canonical(a["bars"])
        adapter={"version":"local_ohlcv_file_input_adapter_result_v1","adapter_version":"local_ohlcv_file_input_adapter_v1","status":"SUCCESS","source_sha256":a["source_artifact_sha256"],"normalized_rows_hash":a["normalized_bars_sha256"],"normalized_bars":copy.deepcopy(a["bars"]),"row_count":len(a["bars"]),"first_timestamp":a["bars"][0]["timestamp"],"last_timestamp":a["bars"][-1]["timestamp"],"downstream_adapter_result":{"synthetic_bars":[{key:value for key,value in bar.items() if key != "volume"} for bar in a["bars"]]},"network_used":False,"provider_calls_used":0,"production_runtime_supported":False}
...
def test_quality_gate_failure_and_review_matrix(case):
    request = _request()
    bars = request["asset_bar_bindings"][0]["adapter_result"]["normalized_bars"]
    ...
    elif case == "no_overlap": request["asset_bar_bindings"][1]["adapter_result"]["normalized_bars"] = _asset("x", "X", start="2025-01-01")["bars"]; expected = "FAILED_VALIDATION"
    elif case == "stale_asset":
        stale = _asset("x", "X", start="2023-01-01")["bars"]; request["asset_bar_bindings"][1]["adapter_result"]["normalized_bars"] = stale; expected = "FAILED_VALIDATION"
